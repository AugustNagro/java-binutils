...
public class Elf implements Closeable {
	...
	public Elf(FileChannel channel) throws IOException {
		...
		// Read the last part of the ELF header and interpret the various
		// headers...
		...
		for (int i = 0; i < programHeaderEntryCount; i++) {
			...
			this.programHeaders[i] = new ProgramHeader(header.elfClass, buf, channelPosition);
		}
		...
		for (int i = 0; i < sectionHeaderEntryCount; i++) {
			...
			SectionHeader sHdr = new SectionHeader(header.elfClass, buf, channelPosition);
			if (i == 0) {
	...
	protected StringBuilder dumpSectionHeader(StringBuilder sb, SectionHeader shdr) {
		...
		sb.append(", size: 0x").append(Long.toHexString(shdr.getSize()));
	 ...
	 * 
	 * @return the first program header with the given type, or
	 *         <code>null</code> if no such segment exists in this ELF object.
	 */
	 ...
	 * this ELF object.
	 * 
	 * @return the name of the interpreter, or <code>null</code> if no
	 *         interpreter could be determined.
	 */
	...
	public ByteBuffer getSection(SectionHeader shdr) throws IOException {
		...
		ByteBuffer buf = ByteBuffer.allocate((int) shdr.getSize());
		buf.order(header.elfByteOrder);
	 ...
	 * 
	 * @return the first section header with the given type, or
	 *         <code>null</code> if no such section exists in this ELF object.
	 */
	 ...
	 * header.
	 * 
	 * @return a {@link ByteBuffer} from which the segment data can be read,
